...
const COLOR_DISTANCE_THRESHOLD_SQ =
...
const estimateBackgroundColor = (
  ...
  const corners = [
    ...
    sampleCorner(
      ctx,
      Math.max(width - CORNER_SAMPLE_SIZE, 0),
      0,
      width,
      height
    ),
    sampleCorner(
      ctx,
      0,
      Math.max(height - CORNER_SAMPLE_SIZE, 0),
      width,
      height
    ),
    sampleCorner(
...
export const backgroundRemovalService = {
  async process(file: File): Promise<BackgroundRemovalResult> {
    ...
    try {
      ...
      throw new Error(
        "Background removal failed. Please try a different image or simpler background."
      );
    }
  },
};
